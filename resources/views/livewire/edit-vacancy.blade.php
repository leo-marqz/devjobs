...
>
    ...
    <div>
        <x-label for="new_image" :value="__('Imagen')" />

        <x-input
            id="new_image"
            class="block mt-1 w-full"
            type="file" wire:model="new_image"
            accept="image/*"
        />
        <div class="my-3">
            <x-label :value="__('Imagen Actual')" />
            <img src="{{asset('storage/vacancies/' . $image)}}" alt="{{'imagen vacante: ' . $image}}">
        </div>
    ...
    <x-button>Editar Vacante</x-button>
